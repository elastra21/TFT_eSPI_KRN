...
class TFT_eSPI_Button {
 ...
 public:
  ...
  uint16_t textcolor, char *label, uint8_t textsize, uint16_t screen, int radius = 2, bool shouldInvert = true);

  // New/alt initButton() uses upper-left corner & size
  void     initButtonUL(TFT_eSPI *gfx, int16_t x1, int16_t y1,
  uint16_t w, uint16_t h, uint16_t outline, uint16_t fill,
  uint16_t textcolor, char *label, uint8_t textsize, uint16_t screen, int radius, bool shouldInvert);
 ...
 private:
  ...
  uint8_t  _textsize, _textdatum, _radius; // Text size multiplier and text datum for button
  uint16_t _outlinecolor, _fillcolor, _textcolor;
  char     _label[20]; // Button text is 9 chars maximum unless long_name used
  uint16_t     _screen;
  bool  currstate, laststate, _shouldInvert; // Button states
};
